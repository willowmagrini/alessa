    ...
    register_post_type( 'eventos',
        array(
            'labels' => array(
                ...
                'add_new'           => 'Adicionar',
                'edit_item'         => 'Modificar evento',
                'update_item'       => 'Atualizar evento',
                'search_items'      => 'Buscar evento',
                'not_found'         => 'Não encontrado',
                'not_found_in_trash'=> 'Nenhum na lixeira',
            ),
        'public'              => true,
        'has_archive'        => true,
...
function ep_eventposts_metaboxes() {
    add_meta_box( 'ept_event_date_start', 'Início do evento', 'ept_event_date', 'eventos', 'side', 'default', array( 'id' => '_start') );
    add_meta_box( 'ept_event_date_end', 'Final do evento', 'ept_event_date', 'eventos', 'side', 'default', array('id'=>'_end') );
    add_meta_box( 'ept_event_location', 'Local do evento', 'ept_event_location', 'eventos', 'side', 'default', array('id'=>'_location') );
}
...
function ept_event_date($post, $args) {
    ...
    echo 'Data de início:<br><input type="text" name="' . $metabox_id . '_day" value="' . $day  . '" size="2" maxlength="2" />';
    echo $month_s;
    echo '<input type="text" name="' . $metabox_id . '_year" value="' . $year . '" size="4" maxlength="4" /><br><br>';
    echo 'Hora do evento:<br><input type="text" name="' . $metabox_id . '_hour" value="' . $hour . '" size="2" maxlength="2"/>:';
...
function ept_event_location() {
    ...
    echo '<label for="_event_location">Local:</label>';
